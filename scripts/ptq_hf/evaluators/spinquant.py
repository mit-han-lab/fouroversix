...
spinquant_optimizer_img = get_image(
    dependencies=[
        Dependency.fast_hadamard_transform,
        Dependency.fouroversix,
        Dependency.spinquant,
...
class SpinQuantOptimizer:
    ...
    def optimize(
        ...
        *,
        ...
        print("Optimized model saved to", spinquant_save_path)
...
class SpinQuantEvaluator(PTQEvaluator):
    ...
    def quantize_model(
        ...
        *,
        ...
        save_path = (
            save_path / "spinquant" / model_name / quantization_config.__hash__()
        )

        # Optimize model first
        if force_requantize_model or not (save_path / "R.bin").exists():
            msg = (
        ...
        # Then, apply GPTQ on rotated weights
        # if not (save_path / "quantized_model.bin").exists():
        sys.argv = [
            sys.argv[0],
            "--input_model",
            model_name,
            ...
            (
                "--load_qmodel_path"
                if (save_path / "quantized_model.bin").exists()
                and not force_requantize_model
                else "--save_qmodel_path"
            ),
            (save_path / "quantized_model.bin").as_posix(),
            *SPINQUANT_ARGS,
        ]
...
def optimize_model(
    ...
    *,
    ...
    if not force_requantize_model and (save_path / "R.bin").exists():
        print(save_path / "R.bin", "exists")
        return

    SpinQuantOptimizer.with_options(gpu=modal_gpu)().optimize_on_modal.remote(
        model_name,
