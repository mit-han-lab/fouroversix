...
class LocalEvaluationCoordinator(BaseEvaluationCoordinator):
    ...
    def __init__(
        ...
        self.database_path = db_path or (FOUROVERSIX_ROOT_DIR / "results.db")
    ...
    def evaluate(
        ...
        **kwargs: dict[str, Any],
        ...
        return evaluator_cls().evaluate(
            ...
            save_path=save_path or (FOUROVERSIX_ROOT_DIR / "ptq"),
            **kwargs,
        )
    ...
    def start( # noqa: C901
        ...
        **kwargs: dict[str, Any],
        ...
        # If there is only one experiment to run, don't use multiprocessing
        if (
            len(model_names) == 1
            and len(ptq_methods) == 1
            and (not run_tasks_in_parallel or len(tasks) == 1)
        ):
            results = self.evaluate(
                model_names[0],
                ptq_methods[0],
                **{**kwargs, "tasks": tasks, "device": device},
            )

            self.save_results(model_names[0], ptq_methods[0], kwargs, results)
            return
        ...
        num_workers = (
            torch.cuda.device_count() // gpus_per_worker if device == "cuda" else 1
        )
